...
If the current environment is not production, will only show just a color rectangle.

= Configuration

To change the output when the environment is not production, add a custom renderer in your application.rb:
  config.ads.renderer = -> { |options|
    tag(
      :img, 
...
{<img src="http://museways.com/logo.png" />}[http://museways.com]
